...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(page) || page < 1 || !Number.isInteger(pageSize) || pageSize < 1 || pageSize > 50) return Response.json({ error: "Invalid pagination" }, { status: 400, headers: jsonHeaders });
    ...
    return Response.json({
      summary: summarizeLifeFinance(records, year, asOfDate),
      transactions: paginateFinanceTransactions(records, page, pageSize),
    }, { headers: jsonHeaders });
  } catch {
